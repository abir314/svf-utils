...
export class Reader {
   ...
   * @returns {Promise<Reader>} Reader for the provided SVF.
   */
  static async FromDerivativeService(
    urn: string,
    guid: string,
    ...
    const accessToken = await authenticationProvider.getToken([
    ...
    async function downloadDerivative(urn: string, derivativeUrn: string) {
      ...
      const downloadInfo = await modelDerivativeClient.getDerivativeUrl(
        accessToken,
        derivativeUrn,
        urn
      );
      const response = await axios.get(downloadInfo.url as string, {
    ...
    const resolve = async (uri: string) => {
      const buffer = await downloadDerivative(
        urn,
        encodeURI(path.normalize(path.join(baseUri, uri)))
      );
      return buffer;
    };
    return new Reader(svf, resolve);
  ...
  protected constructor(
    ...
    const metadata = JSON.parse(
      metadataEntry.getData().toString()
    ) as SVF.ISvfMetadata;
    ...
    zip
      ...
      .filter(
        (entry) =>
          entry.name !== "manifest.json" && entry.name !== "metadata.json"
      )
      .forEach((entry) => {
  ...
  protected findAsset(query: {
    ...
    return this.svf.manifest.assets.find((asset) => {
      return (
        (isNullOrUndefined(query.type) || asset.type === query.type) &&
  ...
  async *enumerateGeometries(): AsyncIterable<SVF.IGeometryMetadata> {
    const geometryAsset = this.findAsset({
      type: SVF.AssetType.GeometryMetadataList,
    });
    if (!geometryAsset) {
  ...
  getMeshPackCount(): number {
    ...
    this.svf.manifest.assets.forEach((asset) => {
      if (
        asset.type === SVF.AssetType.PackFile &&
        asset.URI.match(/^\d+\.pf$/)
      ) {
        count++;
      }
    });
   ...
   */
  async *enumerateMeshPack(
    packNumber: number
  ): AsyncIterable<SVF.IMesh | SVF.ILines | SVF.IPoints | null> {
    const meshPackAsset = this.findAsset({
      type: SVF.AssetType.PackFile,
      uri: `${packNumber}.pf`,
    });
    if (!meshPackAsset) {
  ...
  async readMeshPack(
    packNumber: number
  ): Promise<(SVF.IMesh | SVF.ILines | SVF.IPoints | null)[]> {
    const meshPackAsset = this.findAsset({
      type: SVF.AssetType.PackFile,
      uri: `${packNumber}.pf`,
    });
  ...
  async *enumerateMaterials(): AsyncIterable<SVF.IMaterial | null> {
    const materialsAsset = this.findAsset({
      type: SVF.AssetType.ProteinMaterials,
      uri: `Materials.json.gz`,
    });
    if (!materialsAsset) {
  ...
  async getPropertyDb(): Promise<PropDbReader> {
    ...
    const offsetsAsset = this.findAsset({
      type: SVF.AssetType.PropertyOffsets,
    });
    const avsAsset = this.findAsset({ type: SVF.AssetType.PropertyAVs });
    const attrsAsset = this.findAsset({
      type: SVF.AssetType.PropertyAttributes,
    });
    const valsAsset = this.findAsset({ type: SVF.AssetType.PropertyValues });
    if (!idsAsset || !offsetsAsset || !avsAsset || !attrsAsset || !valsAsset) {
      throw new Error(
        "Could not parse property database. Some of the database assets are missing."
      );
    }
